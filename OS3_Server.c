...
void* client_thread(void* sock_fd) {
	int fd = *(int *)sock_fd;
	if (send(fd, "Hello, world!", 13, 0) == -1) { // send to the client "Hello, world!"
		perror("send");
	}
...
int main(void)
{
	...
	while(1) {  // main accept() loop
		...
		pthread_t ptid;
		void *args = &new_fd;
		pthread_create(&ptid, NULL, &client_thread, args); // creates a thread that runs the function "client_thread"
